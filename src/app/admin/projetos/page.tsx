...
export default async function AdminProjetosPage() {
  ...
  return (
    <div className="flex-1">
      <HeaderBar
        title="Projetos"
        subtitle="Todos os projetos (Admin)"
        hoursWeek={semana}
      />
      <main className="p-6">
            ...
            <tbody>
              {projects.map((p) => (
                <tr key={p.id} className="border-t border-slate-700 hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3 text-slate-400">{p._count.tickets}</td>
                  <td className="px-4 py-3 text-slate-400">{p._count.timeEntries}</td>
                </tr>
              ))}
            </tbody>
          </table>
